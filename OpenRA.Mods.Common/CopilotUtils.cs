	...
	public static class CopilotsUtils
	{
		...
		public static void TryBuildIntent(World world, string buildingName, Actor building, ProductionQueue queue)
		{
			...
			(CPos? Location, int Variant) FindPos(CPos center, CPos target, int minRange, int maxRange)
			{
				...
				if (center != target)
				{
					...
					if (buildingVariantInfo?.Actors != null)
					{
						if (buildingVariantInfo.Facings != null)
						{
							...
							actorVariantLocal = world.LocalRandom.Next(buildingVariantInfo.Actors.Length + 1);
					}
				}
				else
				{
				...
				if (actorVariantLocal != 0)
				{
					variantActorInfo = world.Map.Rules.Actors[buildingVariantInfo.Actors[actorVariantLocal - 1]];
					vbi = variantActorInfo.TraitInfoOrDefault<BuildingInfo>();
				}
			...
			if (location != null)
			{
				OpenRA.Mods.Common.Commands.CopilotBus.Enqueue(new OpenRA.Mods.Common.Commands.IssueOrderIntent
				{
					...
					TargetA = OpenRA.Mods.Common.Commands.TargetSpec.FromCell(location.Value),
